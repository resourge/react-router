...
import { useSearchParams } from '../../hooks/userSearchParams/useSearchParams';
import {
...
> {
	...
	>(
		...
		function flatSearchParams(searchParams: SP, baseKey: string = '', index: number = -1): string[] {
			return (
				...
				.map<Array<string | undefined> | (string | undefined)>((key) => {
					...
					if ( value ) {
						if ( typeof value === 'object' && !Object.keys(value as Record<string, any>).includes('optional') ) {
							return flatSearchParams(value as SP, key).flat();
						}
					}
		...
		_this.searchParamsList = flatSearchParams(searchParams).flat();
